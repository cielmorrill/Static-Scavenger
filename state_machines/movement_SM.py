...
from utils import vec, magnitude, scale, GameScreen
from statemachine import State

class Movement_SM(Abstract_SM):
...
class Acceleration_SM(Movement_SM):
    ...
    def update(self, seconds=0):
        ...
        if self == "positive":
            self.obj.velocity += self.direction * self.obj.speed
        elif self == "negative":
            self.obj.velocity -= self.direction * self.obj.speed
            ...
            if self.obj.velocity[self.axis] > self.accel * seconds:
                self.obj.velocity[self.axis] -= self.obj.speed
            elif self.obj.velocity[self.axis] < -self.accel * seconds:
                self.obj.velocity[self.axis] += self.obj.speed
            else:
                self.obj.velocity[self.axis] = 0
